(function(){
  'use strict';
  const U=window.ReportUI,{M,$,node,clear,num,money,qty,field}=U;
  ...
  function build(){
    ...
    P.mod.forEach((m,i)=>{const box=node('div',undefined,'task');box.append(node('h3',m.name));fields(box,[[`mod.${i}.h`,'采集小时 / 天'],[`mod.${i}.raw`,'原始 GB / 小时'],[`mod.${i}.del`,'成品 GB / 小时']]);b.append(box);});
    fields(b,[['growth','月增长 %',{min:-99,max:100}],['lagDays','成品输出延迟 / 天',{step:1}]]);
    b=section('生命周期与初始库存',true,'天数为数据年龄阈值，不是额外增加的停留天数。最终保留 0 = 无限期。');
    fields(b,[['hotDays','原始热缓冲 / 天',{step:1}],['retain','原始长期留存 %',{max:100}],['stdDays','满多少天转冷层一',{step:1}],['coldTier','冷层一类型',{choices:[['ia','低频'],['arc','供应商冷层一']]}],['arcDays','满多少天转冷层二',{step:1}],['rawDays','原始最终保留 / 天',{step:1}],['prodDays','成品最终保留 / 天',{step:1}],['productTier','成品存储类型',{choices:tiers}],['initialRawPB','已有留存原始数据 / PB'],['initialRawAge','已有原始数据年龄 / 天',{step:1}],['initialProdPB','已有成品 / PB'],['initialProdAge','已有成品年龄 / 天',{step:1}]]);
    ...
    fields(b,[['hotPlace','原始热数据',{choices:place}],['coldPlace','原始冷数据',{choices:place}],['packPlace','成品打包位置',{choices:place}],['productPlace','成品存放位置',{choices:place}]]);
  ...
  function render(){
    try{
      ...
      clear($('kpis'));values.forEach(([label,value,b])=>{const box=node('div',undefined,'metric');box.append(node('div',label,'label'),node('div',value===null?'无交付':money(value),'value'));if(b!==null&&value!==null)box.append(node('div','较基线 '+(value-b>=0?'+':'')+money(value-b),'delta'));$('kpis').append(box);});
      $('flow').textContent='原始流入 '+qty(r.rawGB/30)+'/天 · 成品 '+qty(r.prodGB)+'/月 · 交付 '+qty(r.deliveredGB)+'/月 · 云容量价：'+(M.VENDORS[P.vendor]?.name||'自定义');
      ...
      U.chart($('stockChart'),$('stockLegend'),[['buf','非留存热缓冲'],['std','留存热数据'],['arc','冷层一'],['deep','冷层二'],['prod','成品']].map(([k,name])=>({name,values:results.map(x=>x.stock[k]/divisor)})),results.map(x=>x.month),unit);
      ...
      clear($('ledger'));[
        '当月跨地点搬运 '+qty(r.transferGB)+'，其中按真实转层发生 '+qty(r.migrationGB)+'。成品位置影响输出搬运和客户路径。',
        '本月最短保存期补费 '+money(r.early)+'；原始冷库存读取及恢复副本 '+money(r.recovery)+'。成品取回另外计入读取项。',
  ...
  U.vendors($('vendorTable'));$('version').textContent=' 版本 '+M.VERSION;build();render();
})();
